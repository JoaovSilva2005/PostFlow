...
describe('ChatPage', () => {
  it('exibe o carregamento, gera uma prévia e adiciona o rascunho à agenda', async () => {
    ...
    await user.type(screen.getByLabelText('Pedido para a IA'), 'Post sobre café especial')
    await user.click(screen.getByRole('button', { name: 'Gerar post' }))
    expect(screen.getByText('Criando texto e imagem...')).toBeInTheDocument()

    expect(await screen.findByText('Rascunho gerado', {}, { timeout: 2000 })).toBeInTheDocument()
    ...
    expect(saved[0]).toMatchObject({ date: '2026-08-14', platform: 'Instagram' })
  })
})
