...
const colors = ['#4F46E5', '#F97316', '#16A34A', '#171720']
...
export function BrandPage() {
  ...
  function handleSubmit(event: FormEvent) {
    ...
    saveBrand(form)
    setSaved(true)
    window.setTimeout(() => navigate('/chat'), 500)
  }
  ...
  return (
    <AppShell>
      <header className={styles.pageHeader}>
        ...
        <div className={styles.step}>PASSO 1 DE 3 <strong>Marca</strong></div>
      </header>
        ...
        <form className={styles.formCard} onSubmit={handleSubmit}>
          <div className={styles.cardTitle}>
            ...
            <div>
              ...
              <p>Essas informações serão usadas pela IA para personalizar seus posts.</p>
            </div>
          </div>
              ...
              <div className={styles.colors}>
                {colors.map((color) => (
                  <button
                    ...
                    className={form.primaryColor === color ? styles.selectedColor : ''}
          ...
          <div className={styles.formFooter}>
            <p><Sparkles size={14} /> Você poderá alterar essas informações quando quiser.</p>
            <Button type="submit">{saved ? <><Check size={17} /> Salvo</> : 'Salvar e continuar'}</Button>
          </div>
        </form>

        <aside className={styles.previewCard} aria-label="Prévia da identidade da marca">
          <p className={styles.previewLabel}>PRÉVIA DA MARCA</p>
          <div className={styles.brandMark} style={{ background: form.primaryColor }}>
          ...
          <div className={styles.examplePost}>
            <div className={styles.exampleHeader}>
              ...
              <div><strong>{form.name || 'Sua marca'}</strong><small>agora</small></div>
            </div>
            <p>Conteúdo feito para conversar com seu público de um jeito {form.toneOfVoice.toLowerCase()}.</p>
            <span className={styles.postTag}>#SuaMarca</span>
          </div>
          <p className={styles.previewHint}>A prévia é atualizada enquanto você configura.</p>
        </aside>
      </div>
    </AppShell>
  )
