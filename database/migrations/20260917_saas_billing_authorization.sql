...
-- O BFF chama estas funções com service_role após autorizar membership.
-- A contratação cria uma fatura pendente; a confirmação do pagamento registra
-- receita e snapshot fiscal em uma transação separada e idempotente.
...
declare v_plan public.plans%rowtype; v_subscription_id uuid; v_invoice public.billing_invoices%rowtype;
begin
  select * into v_invoice from public.billing_invoices where brand_id = p_workspace_id and idempotency_key = p_idempotency_key;
  if found then return query select v_invoice.id, v_invoice.invoice_number, v_invoice.amount_cents, v_invoice.status, v_invoice.due_date, v_invoice.paid_at, f.external_reference, f.tax_cents, f.net_cents, f.issued_at from (select 1) seed left join public.fiscal_documents f on f.billing_invoice_id = v_invoice.id; return; end if;
  ...
  insert into public.subscriptions(brand_id,plan_id,status,current_period_start,current_period_end) values(p_workspace_id,v_plan.id,'past_due',current_date,current_date + 30) returning id into v_subscription_id;
  insert into public.billing_invoices(brand_id,subscription_id,invoice_number,amount_cents,status,due_date,paid_at,idempotency_key) values(p_workspace_id,v_subscription_id,'PF-' || to_char(now(),'YYYYMMDD') || '-' || substr(public.gen_random_uuid()::text,1,8),v_plan.price_cents,'pending',current_date + 7,null,p_idempotency_key) returning * into v_invoice;
  ...
  return query select v_invoice.id,v_invoice.invoice_number,v_invoice.amount_cents,v_invoice.status,v_invoice.due_date,v_invoice.paid_at,null::text,null::integer,null::integer,null::timestamptz;
end; $$;
...
begin
  ...
  update public.subscriptions set status='active',updated_at=now() where id=v_invoice.subscription_id;
